Config = {}
Config.allRadarLocations = { -- Add all of your speed camera locations. You can leave some out if you want them to be undetectable.
    {x = 1584.9281,y = -993.4557,z = 59.3923},
	{x = 1258.2006,y = 789.4199,z = 103.2190},
    {x = 2442.2006,y = -134.6004,z = 88.7765},
	{x = 2871.7951,y = 3540.5795,z = 53.0930},
    {x = -47.328,y = -509.6,z = 32.384},
    {x = 2491.465,y = 4110.561,z = 38.219},
    {x = -524.2645,y = -1776.3569,z = 21.3384},
    {x = 130.462,y = -786.304,z = 31.3}
}
Config.distWarning = 100 -- Distance you must be away from a speed camera for the detector to kick in. Default is 100